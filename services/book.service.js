...
function query(filter) {
  let books = _getBooks();
  if (filter) {
    let { nameFilter, minPrice, maxPrice } = filter;
    minPrice = minPrice ? minPrice : 0;
    maxPrice = maxPrice ? maxPrice : Infinity;
    books = books.filter((book) => {
      if (
        book.title.includes(nameFilter) &&
...
function searchBooks(keyword) {
  ...
  const bookApiUrl = `https://www.googleapis.com/books/v1/volumes?q=${keyword}&key=${API_KEY}`;
  return axios.get(bookApiUrl).then((res) =>
...
function addBook(bookId) {
  let bookData;
  return _getBooksFromApi(bookId).then((res) => {
    bookData = res.data;
    console.log("from add book :", bookData);
    debugger
    gBooks.push(
      _createBook(
...
function _getBooksFromApi(bookId) {
  const API_KEY = "AIzaSyC7-PFigwWJ4vca_uLhDjlUOtXhUGwD4zo";
  const bookApiUrl = `https://www.googleapis.com/books/v1/volumes/${bookId}?key=${API_KEY}`;
...
function _getBooks() {
  let books = _loadFromStorage(KEY);
  if (!books) {
    books = gBooks;
...
function _saveToStorage(books=gBooks) {
